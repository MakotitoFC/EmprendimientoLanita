...
export default async function ProductosPage({
  ...
  const [{ data }, { data: ventas }] = await Promise.all([
    query,
  ...
  for (const v of ventas ?? []) {
    ventasPorProducto[v.product_id] = (ventasPorProducto[v.product_id] ?? 0) + v.quantity
  }

  // sort by sales desc, then by created_at
  productos.sort((a, b) => (ventasPorProducto[b.id] ?? 0) - (ventasPorProducto[a.id] ?? 0))

  const tipoLabels: Record<string, string> = { piedra: 'Pieza única', cemento: 'Cemento', mdf: 'MDF' }
    ...
    <div>
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        {['', 'piedra', 'cemento', 'mdf'].map(t => (
          <Link
            key={t}
            href={t ? `?tipo=${t}` : '/dashboard/vendedor/productos'}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-stone-100 bg-stone-50">
                ...
                <th className="text-left px-4 py-3 font-medium text-stone-600 hidden md:table-cell">Ventas</th>
            ...
            <tbody>
              {productos.map(p => (
                <tr key={p.id} className="border-b border-stone-50 hover:bg-stone-50/50 transition-colors">
                  ...
                  <td className="px-4 py-3 hidden md:table-cell text-stone-500">{tipoLabels[p.tipo]}</td>
                  <td className="px-4 py-3 hidden md:table-cell text-stone-700">{formatPrice(p.precio_base)}</td>
                  <td className="px-4 py-3 hidden md:table-cell text-stone-500 font-variant-numeric tabular-nums">
                    {ventasPorProducto[p.id] ? (
                      <span className="font-medium text-stone-700">{ventasPorProducto[p.id]}</span>
                    ) : (
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2 justify-end">
                      <Link href={`/dashboard/vendedor/productos/${p.id}`} className="p-1.5 rounded-lg hover:bg-stone-100 text-stone-500 hover:text-stone-800">
                        <Pencil size={15} />
                      </Link>
